...
#
# NemoClawd sandbox entrypoint. Configures OpenClaw and starts the dashboard
...
- Use `pumpfun/telegram-bot/` as the primary implementation reference for NemoClawd's Pump-Fun Telegram bot runtime.
...
- NemoClawd vault at `~/.nemoclawd/vault/` for append-only JSONL wallet, trade, and heartbeat logs
...
Bundled local references for the Solana NemoClawd environment:
