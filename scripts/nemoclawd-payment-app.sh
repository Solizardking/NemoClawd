...
require_env() {
  local key="$1"
  if [ -z "${!key:-}" ]; then
    echo "[payment-app] Missing required environment variable: $key" >&2; exit 1
  fi
}
...
cd "$APP_DIR"
echo "[payment-app] Starting | Port: ${PORT} | Persona: ${DEFI_AGENT_ID} | Mint: ${AGENT_TOKEN_MINT_ADDRESS}"
