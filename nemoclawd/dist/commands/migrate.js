...
async function cliMigrate(opts) {
    ...
    logger.info("NemoClawd migrate: moving host OpenClaw into OpenShell sandbox");
    ...
    if (!hostState.exists || !hostState.stateDir) {
        ...
        logger.info("Use 'openclaw nemoclawd launch' for a fresh install.");
        return;
    }
    ...
    logger.info("  openclaw nemoclawd status     # Verify everything is healthy");
    ...
    if (skipBackup) {
        ...
        logger.info("  openclaw nemoclawd eject");
    }
}
