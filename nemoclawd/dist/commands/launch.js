...
async function cliLaunch(opts) {
    ...
    logger.info("NemoClawd launch: setting up OpenClaw inside OpenShell");
    ...
    if (!hostState.exists && !force) {
        ...
        logger.info("");
        logger.info("To proceed with NemoClawd-driven bootstrap anyway, use --force.");
        return;
    }
    if (hostState.exists && !force) {
        logger.info("Existing OpenClaw installation detected. Consider using 'openclaw nemoclawd migrate' instead.");
    ...
    logger.info("  openclaw nemoclawd connect    # Enter the sandbox");
    logger.info("  openclaw nemoclawd status     # Check health");
