...
/**
 * NemoClawd — OpenClaw Plugin for OpenShell
 *
...
export default function register(api: OpenClawPluginApi): void {
  ...
  api.registerCommand({
    name: "nemoclawd",
    description: "NemoClawd sandbox management (status, eject).",
    acceptsArgs: true,
    handler: (ctx) => handleSlashCommand(ctx, api),
  });

  // 2. Register `openclaw nemoclawd` CLI subcommands (commander.js)
  api.registerCli(
    (cliCtx) => {
  ...
  api.logger.info("  │  Commands:  openclaw nemoclawd <command>             │");
  api.logger.info("  └─────────────────────────────────────────────────────┘");
  api.logger.info("");
}
